...
| `18.x` | 18 | Hydrogen (Maintenance) - **Current** | lts/hydrogen | `docker pull jch254/docker-node-terraform-aws:18.x` |
...
* Prefer a pinned major tag (e.g. `18.x`, `22.x`) or digest for deterministic CI builds.

`latest` generally tracks the highest actively maintained major (currently Node 18). Pin a major tag (e.g. `18.x`) for stability in CI.
...
```bash
docker run -it --rm jch254/docker-node-terraform-aws:18.x bash
```

Check installed versions:

```bash
docker run --rm jch254/docker-node-terraform-aws:18.x "node --version && terraform version && aws --version"
```
...
```yaml
image: jch254/docker-node-terraform-aws:18.x
...
Configure the project to use the public image `jch254/docker-node-terraform-aws:18.x` (or another tag) as a custom image.
