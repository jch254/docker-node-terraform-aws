...
|------|-------------------|-------|
| Node.js | 20 (alpine base) | Other major versions via branches/tags (below) |
...
| `20.x` | 20 | Iron (Active LTS) - **Current** | lts/iron | `docker pull jch254/docker-node-terraform-aws:20.x` |
...
* Prefer a pinned major tag (e.g. `20.x`, `22.x`) or digest for deterministic CI builds.

`latest` generally tracks the highest actively maintained major (currently Node 20). Pin a major tag (e.g. `20.x`) for stability in CI.
...
```bash
docker run -it --rm jch254/docker-node-terraform-aws:20.x bash
```

Check installed versions:

```bash
docker run --rm jch254/docker-node-terraform-aws:20.x "node --version && terraform version && aws --version"
```
...
```yaml
image: jch254/docker-node-terraform-aws:20.x
...
Configure the project to use the public image `jch254/docker-node-terraform-aws:20.x` (or another tag) as a custom image.
